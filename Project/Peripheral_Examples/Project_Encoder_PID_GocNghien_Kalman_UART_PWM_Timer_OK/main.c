...
/* Global variables ----------------------------------------------------------*/
	...
	uint16_t Sample_time = 400;  // so lan lay mau trong 1s
	uint16_t TimeDelay ;
	
	extern int32_t 	ACCEL_ZOUT;
	extern int32_t 	ACCEL_YOUT;
	extern int32_t 	ACCEL_XOUT;
	extern float 		ACCEL_XANGLE;
	extern float		ACCEL_YANGLE;
	extern float		ACCEL_ZANGLE;
	extern int32_t 	GYRO_XRATE;
	extern float 		angle;
/* Private functions ---------------------------------------------------------*/
/**
  * @brief  Main program
...
//== Calculate Angle
	...
	Get_Accel_Angles();
	Get_GYRO_RATE();
	Com_Angle=Kalman(ACCEL_XANGLE, GYRO_XRATE/16.4);
//Err ===========================
//	if(Com_Angle<0)
//		Err= -(900 + ACCEL_ZANGLE);
//	else if(Com_Angle>0)
//		Err= 900 - ACCEL_ZANGLE;	
//	else
		Err=Com_Angle;
