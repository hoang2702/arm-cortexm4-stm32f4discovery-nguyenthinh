		...
		if (c!='e')
		 {
				if (UART3_count==0)
				{
					 memset(UART3_chuoi, '\0', sizeof(UART3_chuoi));
				}
				UART3_chuoi[UART3_count]=c;
				UART3_count++;
		 }
	  else
		 {
		...
		if (c!='e')
		 {
				if (UART6_count==0)
				{
					 memset(UART6_chuoi, '\0', sizeof(UART6_chuoi));
				}
				UART6_chuoi[UART6_count]=c;
				UART6_count++;
		 }
	  else
		 {
				UART6_count=0;
				//printf("So gui: %d \n",atoi(UART_chuoi));
				USART_puts(USART6,"Done!!");
				USART_puts(USART6,UART6_chuoi);
		 }
